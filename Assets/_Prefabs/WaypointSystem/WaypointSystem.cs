...
public class WaypointSystem : MonoBehaviour
{
    ...
    [HideInInspector]
    public Waypoint activeWaypointTarget;
    ...
    private void initWaypoints()
    {
        ...
        for (int i = 0; i < waypoints.Count; i++)
        {
            ...
            waypoints[i].gizmoSize =
                waypoints[i].origin == waypoints[i].location ? gizmoSize : waypoints[i].gizmoSize;

            waypoints[i].neighborNext = i == waypoints.Count - 1 ? waypoints[0] : waypoints[i + 1];

            waypoints[i].neighborPrevious =
                i == 0 ? waypoints[waypoints.Count - 1] : waypoints[i - 1];
    ...
    private void gizmoDrawLines(Waypoint[] waypointArray)
    {
        ...
        {
            Gizmos.DrawLine(
                waypointArray[i].gameObject.transform.position,
                waypointArray[i + 1].gameObject.transform.position
            );
        }
    }
}
