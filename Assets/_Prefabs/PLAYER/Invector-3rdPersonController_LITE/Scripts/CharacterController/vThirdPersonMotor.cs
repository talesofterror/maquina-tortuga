...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : MonoBehaviour
    {
        ...
        internal PhysicsMaterial frictionPhysics,
            maxFrictionPhysics,
            slippyPhysics; // create PhysicMaterial for the Rigidbody
        ...
        internal float colliderRadius,
            colliderHeight; // storage capsule collider extra information
        ...
        public void Init()
        {
            ...
            frictionPhysics.staticFriction = .5f;
            frictionPhysics.dynamicFriction = .5f;
        ...
        public virtual void SetControllerMoveSpeed(vMovementSpeed speed)
        {
            if (speed.walkByDefault)
                moveSpeed = Mathf.Lerp(
                    ...
                    speed.movementSmooth * Time.fixedDeltaTime
                );
            else
                moveSpeed = Mathf.Lerp(
                    moveSpeed,
        ...
        public virtual void MoveCharacter(Vector3 _direction)
        {
            ...
            inputSmooth = Vector3.Lerp(
                inputSmooth,
                input,
                (isStrafing ? strafeSpeed.movementSmooth : freeSpeed.movementSmooth)
                    * Time.fixedDeltaTime
            );

            if (!isGrounded || isJumping)
            {
                freeSpeed.rotationSpeed = freeSpeed.airborneRotationSpeed;
                return;
            }
            else { }
        ...
        public virtual void CheckSlopeLimit()
        {
            ...
            if (
                Physics.Linecast(
                    transform.position + Vector3.up * (_capsuleCollider.height * 0.5f),
                    transform.position
                        + moveDirection.normalized * (_capsuleCollider.radius + 0.2f),
                    out hitinfo,
                    groundLayer
                )
            )
            {
                ...
                var targetPoint =
                    hitinfo.point + moveDirection.normalized * _capsuleCollider.radius;
                if (
                    (hitAngle > slopeLimit)
                    && Physics.Linecast(
                        transform.position + Vector3.up * (_capsuleCollider.height * 0.5f),
        ...
        public virtual void AirControl()
        {
            ...
            {
                _rigidbody.AddForce(
                    moveDirection * airSpeed * Time.fixedDeltaTime,
            ...
            Vector3 targetPosition =
                _rigidbody.position + (moveDirection * airSpeed) * Time.fixedDeltaTime;
            Vector3 targetVelocity = (targetPosition - _rigidbody.position) / Time.fixedDeltaTime;

            targetVelocity.y = _rigidbody.linearVelocity.y;
            _rigidbody.linearVelocity = Vector3.Lerp(
                _rigidbody.linearVelocity,
                targetVelocity,
                airSmooth * Time.fixedDeltaTime
            );
        }
        ...
        protected virtual void CheckGround()
        {
            ...
            if (groundDistance <= groundMinDistance)
            {
                ...
                if (!isJumping && groundDistance > 0.05f)
                    _rigidbody.AddForce(
                        transform.up * (extraGravity * 2 * Time.deltaTime),
                        ForceMode.VelocityChange
                    );
            ...
            else
            {
                if (groundDistance >= groundMaxDistance)
                {
                    ...
                    {
                        _rigidbody.AddForce(
                            transform.up * extraGravity * Time.fixedDeltaTime,
                            ForceMode.VelocityChange
                        );
                    }
                }
                ...
                {
                    _rigidbody.AddForce(
                        transform.up * (extraGravity * 2 * Time.fixedDeltaTime),
                        ForceMode.VelocityChange
                    );
                }
            }
        ...
        protected virtual void CheckGroundDistance()
        {
            if (_capsuleCollider != null)
            {
                ...
                // ray for RayCast
                Ray ray2 = new Ray(
                    _rigidbody.position + new Vector3(0, colliderHeight / 2, 0),
                    Vector3.down
                );
                // raycast for check the ground distance
                if (
                    Physics.Raycast(ray2, out groundHit, (colliderHeight / 2) + dist, groundLayer)
                    && !groundHit.collider.isTrigger
                )
                    dist = _rigidbody.position.y - groundHit.point.y;
                // sphere cast around the base of the capsule to check the ground distance
                if (dist >= groundMinDistance)
                {
                    Vector3 pos = _rigidbody.position + Vector3.up * (_capsuleCollider.radius);
                    ...
                    if (
                        Physics.SphereCast(
                            ...
                            radius,
                            out groundHit,
                            _capsuleCollider.radius + groundMaxDistance,
                            groundLayer
                        ) && !groundHit.collider.isTrigger
                    )
                    {
                        Physics.Linecast(
                            groundHit.point + (Vector3.up * 0.1f),
                            groundHit.point + Vector3.down * 0.15f,
                            out groundHit,
                            groundLayer
                        );
                        float newDist = _rigidbody.position.y - groundHit.point.y;
                        if (dist > newDist)
                            dist = newDist;
                    }
                }
        ...
        public class vMovementSpeed
        {
            ...
            public float airborneRotationSpeed = 5.5f;

            [Tooltip("Rotation while the character is jumping")]
