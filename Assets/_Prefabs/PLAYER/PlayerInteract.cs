...
public class PlayerInteract : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            if (
                rayHitInteractable.transform.CompareTag("Interactable") && !isTargettingInteractable
            )
            {
                currentInteractable =
                    rayHitInteractable.transform.GetComponentInParent<Interactable>();
    ...
    {
        if (
            Vector3.Distance(PLAYERSingleton.i.transform.position, targetPosition) < raycastDistance
