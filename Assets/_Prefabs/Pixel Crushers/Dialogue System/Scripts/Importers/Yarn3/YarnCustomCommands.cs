...
namespace PixelCrushers.DialogueSystem.Yarn3
{
    public class YarnCustomCommands : MonoBehaviour
    {
        ...
        [SerializeField] private bool supportSmartVariables = false;
        ...
        public LineParser LineParser { get; private set; } = new LineParser();
        ...
        public virtual void RegisterFunctions()
        {
            ...
            // eval() function for smart variables:
            if (supportSmartVariables)
            {
                Lua.environment.Register(nameof(eval), eval);
            }
        }
        ...
        public virtual void UnregisterFunctions()
        {
            ...
            Lua.UnregisterFunction(nameof(eval));
        }
        ...
        public virtual void OnConversationLine(Subtitle subtitle)
        {
            ...
            if (!string.IsNullOrEmpty(subtitle.formattedText.text?.Trim()))
            {
                ...
                var markupResult = LineParser.ParseString(text, Localization.language);
        ...
        public static Language.Lua.LuaValue eval(Language.Lua.LuaValue[] values)
        {
            var code = "return " + values[0].ToString().Replace(@"\'", "'");
            var returnValue = Lua.Run(code, true, true).luaValue;
            return returnValue;
        }
